...
class Server {
    ...
    loadAccessories() {
        ...
        this.config.accessories.forEach((accessoryConfig, index) => {
            ...
            if (accessoryConfig._bridge) {
                ...
                try {
                    this.validateChildBridgeConfig("platform" /* PLATFORM */, accessoryIdentifier, accessoryConfig._bridge);
                }
                catch (error) {
                    log.error(error.message);
                    return;
                }
                const childBridge = new childBridgeService_1.ChildBridgeService("accessory" /* ACCESSORY */, accessoryIdentifier, plugin, accessoryConfig, accessoryConfig._bridge, this.config, this.options, this.api, this.ipcService, this.externalPortService);
    ...
    loadPlatforms() {
        ...
        this.config.platforms.forEach((platformConfig, index) => {
            ...
            if (platformConfig._bridge) {
                ...
                const childBridge = new childBridgeService_1.ChildBridgeService("platform" /* PLATFORM */, platformIdentifier, plugin, platformConfig, platformConfig._bridge, this.config, this.options, this.api, this.ipcService, this.externalPortService);
    ...
    validateChildBridgeConfig(type, identifier, bridgeConfig) {
        ...
        if (this.childBridges.has(bridgeConfig.username)) {
            throw new Error(`Error loading the ${identifier} "${identifier}" requested in your config.json - ` +
